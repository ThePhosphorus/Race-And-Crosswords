...
describe("Grid generation", () => {
    ...
    describe("When the grid is generated (easy)", () => {
        ...
        it("should have correct definition", async () => {
            for (const word of grid.words) {
                const datamuseWord: DatamuseWord = await communication.getDefinitionsFromServer(word.toString());
                assert.notEqual(datamuseWord, null, word.toString() + " does not exist");
                assert.notEqual(datamuseWord.defs, null, word.toString() + " does not have defs");
        ...
        it("should have easy words", async () => {
            const datamuseWord: DatamuseWord = await communication.getDefinitionsFromServer(grid.words[0].toString());
